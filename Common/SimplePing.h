...
typedef NS_ENUM(NSInteger, SimplePingAddressStyle) {
    ...
    SimplePingAddressStyleICMPv4 NS_SWIFT_NAME(icmpv4),       ///< Use the first IPv4 address found.
    
    SimplePingAddressStyleICMPv6 NS_SWIFT_NAME(icmpv6)        ///< Use the first IPv6 address found.
};
...
- (void)sendPingWithData:(nullable NSData *)data NS_SWIFT_NAME(sendPing(data:));
...
- (void)simplePing:(SimplePing *)pinger didStartWithAddress:(NSData *)address NS_SWIFT_NAME(simplePing(_:didStart:));
...
- (void)simplePing:(SimplePing *)pinger didFailWithError:(NSError *)error NS_SWIFT_NAME(simplePing(_:didFailed:));
